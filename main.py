...
async def status(interaction: discord.Interaction, report_id: int, new_status: app_commands.Choice[str]):
    ...
    try:
        ...
        # 埋め込みのフィールドを更新（ステータスフィールドを探して値を変更）
...
@app_commands.choices(violated_rule=[
...
async def report(interaction: discord.Interaction, target_user: discord.User, violated_rule: app_commands.Choice[str], details: str = None, message_link: str = None):
    ...
    if REPORT_CHANNEL_ID == 0:
        await interaction.response.send_message("現在、ボットが設定中のため通報機能を利用できません。管理者にお知らせください。", ephemeral=True)
        return

    try:
        report_id = await db.create_report(
            interaction.guild.id, target_user.id, violated_rule.value, details, message_link
        )

        report_channel = client.get_channel(REPORT_CHANNEL_ID)
        if not report_channel:
            await interaction.response.send_message("報告用チャンネルが見つかりません。管理者に設定を確認するよう伝えてください。", ephemeral=True)
            return

        embed = discord.Embed(
            title=f"🚨 新規の匿名報告 (ID: {report_id})",
            description="サーバーの平和を守るためのご協力、ありがとうございます。",
        ...
        sent_message = await report_channel.send(embed=embed)
